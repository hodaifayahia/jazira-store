...
export default function AdminVariationsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!formType.trim() || !formValue.trim()) throw new Error('النوع والقيمة مطلوبان');
      const payload = {
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="font-cairo text-sm text-muted-foreground mt-1">
            {options?.length || 0} متغير — أضف الألوان والمقاسات وأنواع أخرى ثم اربطها بالمنتجات
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-card border rounded-xl">
          ...
          <p className="font-cairo text-muted-foreground">لا توجد متغيرات بعد</p>
              ...
              <div className="divide-y">
                {items.map(o => (
                  <div key={o.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors group">
                    ...
                    {o.color_code ? (
                      <div
                        className="w-8 h-8 rounded-lg border-2 border-muted-foreground/20 shrink-0"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {o.color_code && (
                        <span className="font-roboto text-xs text-muted-foreground mr-2">{o.color_code}</span>
                      )}
                    </div>
          ...
          <div className="space-y-4 pt-2">
            <div>
              ...
              <Input
                ...
                placeholder="مثل: اللون، المقاس، النوع"
            ...
            <div>
              ...
              <Input value={formValue} onChange={e => setFormValue(e.target.value)} placeholder="مثل: أحمر، XL، قطن" className="font-cairo mt-1.5" />
            </div>

            {/* Color picker - show when type looks like a color */}
            {isColorType(formType) && (
              <div>
                <Label className="font-cairo text-sm">كود اللون</Label>
                <div className="flex items-center gap-3 mt-1.5">
                  <input
                    type="color"
                    value={formColorCode || '#000000'}
                  ...
                  <div
                    ...
                    style={{ backgroundColor: formColorCode || '#000' }}
                  />
                </div>
              </div>
            )}
